...
with open(output_sql, 'w') as sql_file:
    ...
    # Drop and Create DimVehicle Table
    sql_file.write("""
...
PRINT 'DimVehicle table created successfully';
\n\n
""")

    # Drop and Create DimFuel Table
    sql_file.write("""
IF OBJECT_ID('dbo.DimFuel', 'U') IS NOT NULL
    DROP TABLE dbo.DimFuel;
CREATE TABLE dbo.DimFuel (
    FuelID INT IDENTITY(1,1) PRIMARY KEY,
    FuelType NVARCHAR(50),
    FuelPrice FLOAT,
    Date DATE
);
PRINT 'DimFuel table created successfully';
\n\n
""")

    # Drop and Create DimEmissions Table
    sql_file.write("""
IF OBJECT_ID('dbo.DimEmissions', 'U') IS NOT NULL
    DROP TABLE dbo.DimEmissions;
CREATE TABLE dbo.DimEmissions (
    EmissionID NVARCHAR(50) PRIMARY KEY,
    VehicleID INT,
    EmissionStandard NVARCHAR(50),
    SmogRating INT,
    SalesArea NVARCHAR(50),
    FOREIGN KEY (VehicleID) REFERENCES dbo.DimVehicle(VehicleID)
);
PRINT 'DimEmissions table created successfully';
\n\n
""")

    # Drop and Create DimTime Table
    sql_file.write("""
...
PRINT 'DimTime table created successfully';
\n\n
""")
    ...
    sql_file.write("""
IF OBJECT_ID('dbo.FactFuelEconomy', 'U') IS NOT NULL
    DROP TABLE dbo.FactFuelEconomy;
CREATE TABLE dbo.FactFuelEconomy (
...
PRINT 'FactFuelEconomy table created successfully';
\n\n
""")
    ...
    for _, row in vehicles_df.iterrows():
        ...
        sql_file.write(
            f"INSERT INTO dbo.DimVehicle VALUES ({row['id']}, '{make}', '{model}', '{base_model}', "
    ...
    for _, row in gasoline_df.iterrows():
        ...
        sql_file.write(
            f"INSERT INTO dbo.DimFuel (FuelType, FuelPrice, Date) VALUES ('Regular', {fuel_price}, '{row['Day']}');\n"
        )
    ...
    for _, row in diesel_df.iterrows():
        ...
        sql_file.write(
            f"INSERT INTO dbo.DimFuel (FuelType, FuelPrice, Date) VALUES ('Diesel', {fuel_price}, '{row['Day']}');\n"
        )
    ...
    for _, row in emissions_df.iterrows():
        ...
        sql_file.write(
            f"INSERT INTO dbo.DimEmissions VALUES ('{emission_id}', {vehicle_id}, '{emission_standard}', {smog_rating}, '{sales_area}');\n"
        )
    ...
    for _, row in vehicles_df.iterrows():
        try:
            ...
            quarter = (month - 1) // 3 + 1
            sql_file.write(f"INSERT INTO dbo.DimTime VALUES ('{parsed_date.date()}', {year}, {month}, {day}, {quarter});\n")
    ...
    for _, row in vehicles_df.iterrows():
        ...
        sql_file.write(
            f"INSERT INTO dbo.FactFuelEconomy VALUES ({row['id']}, {fuel_cost}, {city_mpg}, "
            f"{highway_mpg}, {combined_mpg}, {co2_emissions}, {smog_score}, '{record_date}');\n"
        )

    # End of script
    sql_file.write("-- End of SQL Script\n")

print("SQL script has been successfully generated.")
